...
import groups
from groups import GROUP_SIZES, get_group_sizes, get_one_factor, is_factor, make_groups


def test_get_group_sizes():
    group_size, special_group_size = get_group_sizes(12)
    assert group_size == 3
    assert special_group_size is None
    ...
    with pytest.raises(ValueError):
        groups.get_group_sizes(None)

    with pytest.raises(ValueError):
        groups.get_group_sizes(-1)

    with pytest.raises(ValueError):
        groups.get_group_sizes(2)
...
def test_get_one_factor_no_factors():
    assert get_one_factor(29, GROUP_SIZES) is None
